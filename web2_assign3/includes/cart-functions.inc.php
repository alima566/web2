...
function outputCartPaintings ($results, $qty, $frame, $glass, $matt)
{
    ...
    foreach ($results as $row)
    {
        ...
        echo '<img src="images/art/works/square-medium/' . $row['ImageFileName'] . '.jpg" alt="..." title="' . $row['Title']. '" id="hover"/></a>';
        echo '</div>'; // image
        ...
        echo '<td><div class="ui form">
              <input type="number" name="qty[]" value="' . $qty .'" min="0" id="qty">
              </div></td>';
        echo '<td class="center aligned" id="price">$' . number_format($row['MSRP']) . '</td>';
        echo '<td class="center aligned" id="addon">$' . number_format($totalAddOn) . '</td>';
        echo '<td class="center aligned" id="total">$' . number_format(getBaseCost($qty, $row['MSRP'])) . '</td>';
        echo '</tr>';
    }
}
...
function printButtons ()
{
    ...
    echo '<a href="index.php"><button type="button" class="ui center aligned fluid blue button">Continue Shopping</button></a>';
